...
const FormsPage = () => {
  ...
  const validationHandler = (): boolean => {
    ...
    if(formState.email === '') {
      ...
      setInvalidFields(prevstate => [...prevstate].filter(err => err !== 'Email'))
  ...
  return (
    <>
      <FormContext.Provider value={{firstName: formState.firstName, email: formState.email, emailIsValid: enteredEmailIsValid, firstNameIsValid: enteredFirstNameIsValid}}>
        <MainHeader />
        <section style={{marginTop: '120px'}}>
          ...
          {/* <UserInfo/> */}
          {invalidFields.length > 0 && <ErrorPage invalidFields={invalidFields}/>}
